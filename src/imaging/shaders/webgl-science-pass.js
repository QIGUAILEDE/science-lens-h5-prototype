...
void main() {
  ...
  if (u_family == 1) {
    ...
    float positiveHalo = smoothstep(0.008, 0.08, dog) * 0.34;
    float negativeHalo = smoothstep(0.006, 0.075, -dog) * 0.26;
    float phase = base + positiveHalo - negativeHalo + edge * 0.18 * u_structureAmount;
    result = mix(vec3(phase), vec3(0.62, 0.70, 0.74) * phase + 0.08, 0.36);
  } else if (u_family == 2) {
    float subjectMask = smoothstep(u_blackLevel + 0.1, u_blackLevel + 0.58, smooth2);
    float signalA = pow(max(smooth2 - u_blackLevel, 0.0), 1.18) * subjectMask;
    float signalB = pow(clamp(detail * 5.2 + edge * 2.4, 0.0, 1.0), 1.35) * smoothstep(0.04, 0.38, y);
    vec3 green = u_channelA * signalA;
    vec3 magenta = u_channelB * signalB;
    vec3 bloom = (green + magenta) * highlight * u_bloom * 1.4;
    result = green + magenta + bloom;
    result *= darkMask;
  } else if (u_family == 3) {
    ...
    float micro = clamp(detail * 6.5 + edge * 3.2, 0.0, 1.0);
    float surface = y * 0.74 + directional * 0.34 + micro * u_structureAmount;
    ...
    vec3 stain = mix(u_stainA, u_stainB, clusterA);
    vec3 diffused = mix(stain, stain * (0.78 + clusterB * 0.42), textureGate);
    result = mix(vec3(y), diffused, 0.42 * u_intensity);
...
export class WebGLSciencePass {
  ...
  render(sourceCanvas, style, state) {
    ...
    setUniforms(gl, this.locations, style, state, width, height);
...
function setUniforms(gl, locations, style, state, width, height) {
